...
data <- data.frame(company = c("A","B","C"), sr =c(.6, .8, .83), prior = c(3/5,1/5,1/5))

# This function takes a data frame that has at least 3 columns, for the candidate (name first column and rate second column) and the prior (third column), a number of trials N and a number of success s.
...
#### Answer
```{r}
# Yesterday's posterior is today's prior (this considers all the info from the 20 reviews), ansd computing the likelihood of ony the 10 most recent reviews
data_1 <- data[,1:3]
...
```{r}
data <- data.frame(factory = c("A","B","C"), dr =c(.1, .15, .2), prior = c(.5,.25,.25))
# Observation is that one defect occurred, so the belief is updated 
data_1 <- updating(data, 1, 1)
data_1
# Does A, or B, or C have a defect in the next shipment? Is calcualted as an expected value 
...
```{r}
# By drawing a garden of forking data, and setting the prior probabilities as the posterior probabilities calculated in the last step
...
```{r}
# Since we include the defect, that is we have seen a defect and updated the beliefs, the prior is the posterior of the last point, and the likelihood is the probability that the ML algorithm gives A given that A is true (that if A really made the product the ML shows A), which is 0.9
...
I chose a beta distribution with a mean close to 0.3 because the "general knowledge" is the earth is 70% water, but I considered values lower than 0.3 were more likely because of the melting of ice caps which could increase the percentage of water that covers the surface. 
...
ggplot(summary) +
  ...
  geom_density(data = sample, aes(x = smp), color = "green", size = 1) + 
  labs(x = expression(theta), 
       y = "Density", title = "Prior distribution and sampling from it")
...
```{r}
# Plotting the data generated with the prior first
# Sample from prior
N <- 1e3
...
for (i in seq_along(sample$smp)){ 
  preds[i, "L"] <- rbinom(n = 1, size = N, prob = sample[i, "smp"])
}
...
preds %>% ggplot(aes(x=L)) + 
  ...
  labs(x = "Number of Simulated L out of 1000",
       y = "Frequency", title = "Values obtained when sampling with the prior") 

```


Now, updating with the new observations
...
for (i in seq_along(post_sample$smp)){ 
  preds[i, "L"] <- rbinom(n = 1, size = N, prob = post_sample[i, "smp"])
}
...
preds %>% ggplot(aes(x=L)) + 
  ...
  scale_x_continuous(limits = c(0,N), breaks = seq(0,N,100)) + 
  labs(x = "Number of Simulated L out of 1000",
       y = "Frequency", title = "Values obtained when sampling with the posterior") 
