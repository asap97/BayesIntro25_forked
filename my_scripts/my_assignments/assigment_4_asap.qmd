...
Given that both intervals exclude 0, there is a credible difference in the risk of developing CHD given a preexisting condition of diabetes for both men and women. Specifically, diabetes increases the risk, since the difference of probabilities is positive in both cases.  
...
I also centered the data before, instead of calculating the average and using it inside the ulam() funciton, because I was getting an error when trying to do that. 
...
m2_gender_index <- ulam(
  ...
  chains = 4, 
  cores = 4, 
  file = here("my_scripts/my_models", "assignment4_m2_gender_index")
)
...
m2_gender_indicator <- ulam(
  ...
  chains = 4, 
  cores = 4, 
  file = here("my_scripts/my_models", "assignment4_m2_gender_indicator") 
)
```
...
Given the positive value of the coefficient for the variable, age does increase the risk for  men and women. For each year above the average age, the risk increases by 50% for both genders. 
