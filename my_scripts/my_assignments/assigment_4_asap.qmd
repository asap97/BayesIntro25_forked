...
I also centered the data before, instead of calculating the average and using it inside the ulam() funciton, because I was getting an error when trying to do that. The age is averaged with respect to all the individuals in the sample. 
...
Given the positive value of the coefficient for the variable, age does increase the risk for  men and women. For each year above the average age, the risk increases by around 51% for both genders. 
