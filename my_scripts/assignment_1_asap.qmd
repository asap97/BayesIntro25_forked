...
```{r}
# i is the number of times that land is observed, so 10-i is the number of times water is observed
...
ggplot(df, aes(x = land, y = probs)) +
  geom_line(size = 1.2) +
  labs(title = "Probability of the proportion of land being 0.3",
...
cp_seq <- seq(0, 1, 0.1)
land <- c(0:10)
prob_df <- data.frame(matrix(0, nrow=length(land), ncol=length(cp_seq)+1))
colnames(prob_df) <- c("land", as.character(cp_seq))
prob_df$land <- land

for (cp in cp_seq) {
  probs <- (cp^obs) * ((1 - cp)^(10 - obs))
  probs <- probs / sum(probs)
  probs <- round(probs, 3)
  prob_df <- cbind(prob_df,probs)
  }

colnames(prob_df) <- c("land",cp_seq)
prob_df
```
